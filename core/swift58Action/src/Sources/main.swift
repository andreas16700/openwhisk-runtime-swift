...
let jsonData = try JSONSerialization.data(withJSONObject: parsed["value"] as Any, options: [])
       await _run_main(mainFunction: GetSourceData, json: jsonData)
   }catch{
_whisk_print(message: "either no input given or input given wasn't valid json!", title: "running with no input")
       await _run_main(mainFunction: GetSourceData, json: nil)
   }
}
